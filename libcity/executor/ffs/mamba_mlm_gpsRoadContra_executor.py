...
# from libcity.model.ffs import MambaFuseAugView
...
class MambaMlmGpsRoadContraExecutor(object):
    '''
    ...
    def __init__(self, config, model, roadgat_data):
        ...
        # self._logger.info(f"Mamba Fuse View Executor : 加载预训练的MambaRoadView")
        # checkpoint = torch.load(config['pretrain_mamba_road_view_ckpt'], map_location='cpu')
        # self.mambaRoadView = checkpoint['model'].to(config['device'])
        # 打印基本信息
        ...
        #
        self._logger.info("⚠️ 注意，这里的MambaFuseViewExecutor不使用常规evalute，所以这里不使用evaluator")
        ...
        # 设置对齐损失和均匀损失的权重、参数，以及是否在训练和测试中使用这些损失。
        ...
        self.train_align_uniform = self.config.get("train_align_uniform", False)  # False
        self.test_align_uniform = self.config.get("test_align_uniform", True)  # True
        self.norm_align_uniform = self.config.get("norm_align_uniform", False)  # False

        self.mlm_ratio = self.config.get('mlm_ratio', 0.6)
        self.rm_ratio = self.config.get("rm_ratio", 0.2)
        self.gc_ratio = self.config.get("gc_ratio", 0.2)
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        for epoch_idx in range(start_epoch, self.max_epochs):
            ...
            train_avg_loss, train_avg_acc, train_gc_avg_loss, train_rm_avg_loss = self._train_epoch(train_dataloader, epoch_idx) # , train_gc_avg_loss
            t1 = time.time()
            train_time.append(t1 - start_time)
            train_loss.append(train_avg_loss)
            train_acc.append(train_avg_acc)
            train_rm_loss.append(train_rm_avg_loss)
            train_gc_loss.append(train_gc_avg_loss)
            self._logger.info("epoch complete!")

            self._logger.info("evaluating now!")
            t2 = time.time()
            eval_avg_loss, eval_avg_acc, eval_gc_avg_loss, eval_rm_avg_loss = self._valid_epoch(eval_dataloader, epoch_idx, mode='Eval')
            end_time = time.time()
            eval_time.append(end_time - t2)
            eval_loss.append(eval_avg_loss)
            eval_acc.append(eval_avg_acc)
            eval_rm_loss.append(eval_rm_avg_loss)
            eval_gc_loss.append(eval_gc_avg_loss)
            ...
            if (epoch_idx % self.log_every) == 0:
                message = 'Epoch [{}/{}] ({})  train_loss: {:.4f}, val_loss: {:.4f}, lr: {:.6f}, {:.2f}s'. \
                    format(epoch_idx, self.max_epochs, (epoch_idx + 1) * num_batches, train_avg_loss, eval_avg_loss, log_lr, (end_time - start_time))
        ...
        self._draw_png([(train_loss, eval_loss, 'loss'), (train_avg_acc, eval_avg_acc, 'avg_loss'), (train_gc_loss, eval_gc_loss, 'gc_loss'), (train_rm_loss, eval_rm_loss, 'rm_loss'), (lr_list, 'lr')])
        return min_val_loss
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            gps_X, gps_padding_mask, \
                road_X, road_Target, road_padding_mask, road_target_masks, road_traj_mat, \
                    aug_gps_X, aug_gps_padding_mask = batch
            road_fuse_pooled, gps_fuse_pooled, predictions_l, road_mean_pooled, gps_mean_pooled, aug_gps_mean_pooled = self.model(
                ...
                aug_gps_X = aug_gps_X, aug_gps_padding_mask = aug_gps_padding_mask,
            ...
            road_match_loss_con = self._contrastive_loss(aug_gps_mean_pooled, road_mean_pooled, self.contra_loss_type)
            gps_contra_loss_con = self._contrastive_loss(gps_mean_pooled, aug_gps_mean_pooled, self.contra_loss_type)
            # road_match_loss_con = self._contrastive_loss(gps_e, road_e, self.contra_loss_type)
            mean_loss = self.mlm_ratio * mean_loss_l + self.gc_ratio * gps_contra_loss_con + self.rm_ratio * road_match_loss_con
            ...
            #     # test 使用，不加入loss反向，测试时评估 embedding 质量
            #     align_uniform_loss, align_loss, uniform_loss = self.align_uniform(gps_mean_pooled, aug_gps_mean_pooled)
            ...
            with torch.no_grad():
                ...
                epoch_rm_loss.append(road_match_loss_con.item())
                epoch_gc_loss.append(gps_contra_loss_con.item())
            ...
            post_fix = {
                ...
                'GPS Contra Loss': gps_contra_loss_con.item(),
                'Road Match Loss': road_match_loss_con.item(),
                # "Mean Loss": mean_loss_con.item(),
            }
            # if self.test_align_uniform or self.train_align_uniform:
        ...
        epoch_rm_loss = np.mean(epoch_rm_loss)
        epoch_gc_loss = np.mean(epoch_gc_loss)
        total_correct_l = total_correct_l / total_active_elements_l * 100.0
        self._logger.info("\n📈 Train: expid = {}, Epoch = {}, avg_loss = {}, total_acc = {} ,avg_gps_contra_loss = {}, avg_road_gps_contra_loss={}" # , avg_gps_contra_loss = {}.
                          .format(self.exp_id, epoch_idx, epoch_loss, total_correct_l , epoch_gc_loss, epoch_rm_loss)) # , epoch_gc_loss
        self._writer.add_scalar('Train loss', epoch_loss, epoch_idx)
        self._writer.add_scalar('Train loc acc', total_correct_l, epoch_idx)
        self._writer.add_scalar('Train road-gps match loss', epoch_rm_loss, epoch_idx)
        self._writer.add_scalar('Train gps-contra match loss', epoch_gc_loss, epoch_idx)
        return epoch_loss, total_correct_l,  epoch_gc_loss, epoch_rm_loss  ## epoch_rm_loss #
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="⏳ {} epoch={}".format(
                ...
                # mean_loss_l, batch_loss_l, num_active_l = self._cal_loss(predictions_l, targets_l, target_masks_l)
                # mean_loss_con = self._contrastive_loss(gps_e, road_e, self.contra_loss_type)
                ...
                road_match_loss_con = self._contrastive_loss(aug_gps_mean_pooled, road_mean_pooled, self.contra_loss_type)
                gps_contra_loss_con = self._contrastive_loss(gps_mean_pooled, aug_gps_mean_pooled, self.contra_loss_type)
                # gps_contra_loss_con = self._contrastive_loss(gps_e, aug_gps_e, self.contra_loss_type)
                # road_match_loss_con = self._contrastive_loss(gps_e, road_e, self.contra_loss_type)
                mean_loss = self.mlm_ratio * mean_loss_l + self.gc_ratio * gps_contra_loss_con  + self.rm_ratio * road_match_loss_con

                # mean_loss = self.mlm_ratio * mean_loss_l + self.contra_ratio * mean_loss_con
                # mean_loss = mean_loss_con
            ...
            epoch_gc_loss = np.mean(epoch_gc_loss)
            total_correct_l = total_correct_l / total_active_elements_l * 100.0
            self._logger.info("\n📈 {}: expid = {}, Epoch = {}, avg_loss = {}, total_correct={}, avg_gps_contra_loss = {}, avg_road_contra_loss = {}" # , avg_gps_contra_loss = {}.
                              .format(mode, self.exp_id, epoch_idx, epoch_loss, total_correct_l , epoch_gc_loss, epoch_rm_loss)) # , epoch_gc_loss
            ...
            self._logger.info(f"\n⌚ {mode}: all time = {all_time}, one traj = {all_time/(len(eval_dataloader)*self.batch_size)}")

            if mode == 'Test':
                self.evaluator.save_result(self.evaluate_res_dir)
            return epoch_loss, total_correct_l, epoch_gc_loss, epoch_rm_loss # epoch_rm_loss , epoch_gc_loss #
    ...
    def save_model(self, cache_name):
        ...
        ensure_dir(self.cache_dir)
        config = dict()
        if isinstance(self.model, MambaMlmGpsRoadContra):
            config['model'] = self.model.gps_view.cpu()
