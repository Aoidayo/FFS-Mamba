...
from libcity.model.ffs import FFSTTE
...
class FFSTTE_Executor(object):
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            gps_X, gps_padding_mask, gps_tte_targets = batch # gps_tte_targets (B,1)
            predictions = self.model(gps_X, gps_padding_mask) # (B,1)
    ...
    def save_model(self, cache_name):
        ...
        ensure_dir(self.cache_dir)
        config = dict()
        if isinstance(self.model, FFSTTE):
