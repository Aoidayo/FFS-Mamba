...
class MambaFuseAugViewExecutor(object):
    '''
    ...
    def __init__(self, config, model, roadgat_data):
        ...
        self.rm_ratio = self.config.get("rm_ratio", 0.3)
        self.gc_ratio = self.config.get("gc_ratio", 0.7)
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        for epoch_idx in range(start_epoch, self.max_epochs):
            ...
            train_avg_loss, train_rm_avg_loss, train_gc_avg_loss = self._train_epoch(train_dataloader, epoch_idx)
            t1 = time.time()
            ...
            t2 = time.time()
            eval_avg_loss, eval_rm_avg_loss, eval_gc_avg_loss = self._valid_epoch(eval_dataloader, epoch_idx, mode='Eval')
            end_time = time.time()
        ...
        self._draw_png([(train_loss, eval_loss, 'loss'), (train_gc_loss, eval_gc_loss, 'gc_loss'), (train_rm_loss, eval_rm_loss, 'rm_loss'), (lr_list, 'lr')])
        return min_val_loss
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            road_e, gps_e, aug_gps_e = self.model(
            ...
            mean_loss = self.gc_ratio * gps_contra_loss_con + self.rm_ratio * road_match_loss_con
            ...
            if self.test_align_uniform or self.train_align_uniform:
                ...
                align_uniform_loss1, align_loss1, uniform_loss1 = self.align_uniform(gps_e, road_e)
                align_uniform_loss2, align_loss2, uniform_loss2 = self.align_uniform(gps_e, aug_gps_e)
                align_uniform_loss = self.rm_ratio * align_uniform_loss1 + self.gc_ratio * align_uniform_loss2
                align_loss = self.rm_ratio * align_loss1 + self.gc_ratio * align_loss2
                uniform_loss = self.rm_ratio * uniform_loss1 + self.gc_ratio * uniform_loss2
        ...
        self._logger.info("\n📈 Train: expid = {}, Epoch = {}, avg_loss = {}, avg_road_match_loss = {}, avg_gps_contra_loss = {}."
                          .format(self.exp_id, epoch_idx, epoch_loss, epoch_rm_loss, epoch_gc_loss))
        self._writer.add_scalar('Train loss', epoch_loss, epoch_idx)
        # self._writer.add_scalar('Train loc acc', total_correct_l, epoch_idx)
        return epoch_loss, epoch_rm_loss, epoch_gc_loss  # , total_correct_l
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            ...
            epoch_gc_loss = np.mean(epoch_gc_loss)
            # total_correct_l = total_correct_l / total_active_elements_l * 100.0
            self._logger.info("\n📈 {}: expid = {}, Epoch = {}, avg_loss = {}, avg_road_match_loss = {}, avg_gps_contra_loss = {}."
                              .format(mode, self.exp_id, epoch_idx, epoch_loss, epoch_rm_loss, epoch_gc_loss))
    ...
    def save_model(self, cache_name):
        ...
        ensure_dir(self.cache_dir)
        config = dict()
        if isinstance(self.model, MambaFuseAugView):
    ...
    def load_model_state(self, cache_name):
        ...
        if isinstance(self.model, MambaFuseAugView):
            self.model.gps_view.load_state_dict(checkpoint['model'].state_dict()) # 加载到当前model
    ...
    def load_model(self, cache_name):
        ...
        checkpoint = torch.load(cache_name, map_location='cpu')
        if isinstance(self.model, MambaFuseAugView):
            self.model.gps_view = checkpoint['model'].to(self.device) # 直接加载预训练模型，覆盖model
            self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            self._logger.info("🔥 Loaded model at " + cache_name)
        else:
            self._logger.info("❌ model非MambaFuseView")
    ...
    def load_model_with_epoch(self, epoch):
        ...
        if isinstance(self.model, MambaFuseAugView):
            ...
            self.model.gps_view = checkpoint['model'].to(self.device)
            self.optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
