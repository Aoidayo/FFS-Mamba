...
import pandas as pd # 建议使用 pandas 打印美观的表格，如果没有可改用普通 print

from libcity.model.ffs import MambaDp
# from libcity.model.ffs import FFSDP
...
def haversine_distance(lnglat1, lnglat2):
    """
    lnglat1, lnglat2: (B,2) [lng, lat]
    return: distance in meters, shape (B,)
    """
    lon1, lat1 = torch.deg2rad(lnglat1[:,0]), torch.deg2rad(lnglat1[:,1])
    lon2, lat2 = torch.deg2rad(lnglat2[:,0]), torch.deg2rad(lnglat2[:,1])
...
class MSEOnMeters(nn.Module):
    """
    用 Haversine 计算 pred 与 target 的地理距离 (meters)，
    再对 meters 做 MSE。
    """
    def __init__(self):
        super().__init__()

    def forward(self, predictions, gps_dp_targets):
        # predictions, gps_dp_targets: (B, 2)
        meters = haversine_distance(predictions, gps_dp_targets)  # (B,)
        loss = torch.mean(meters ** 2)
        return loss #, meters  # 返回 loss + 每个点的误差，方便日志
...
class BackwardTimer:
    ...
    def _register_hooks(self):
        ...
        def post_hook(module, grad_input, grad_output):
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            elapsed = time.perf_counter() - module.__start_time
            self.stats[module.__name].append(elapsed)

        # 遍历所有子模块
        for name, module in self.model.named_modules():
            # 只针对底层叶子模块（如 Conv2d, Linear）进行监控
            if len(list(module.children())) == 0:
                module.__name = f"{name} ({module.__class__.__name__})"
                h1 = module.register_full_backward_pre_hook(pre_hook)
                h2 = module.register_full_backward_hook(post_hook)
                self.hooks.extend([h1, h2])

    def print_stats(self):
        """在一个 Epoch 结束后调用"""
        print(f"\n{'=' * 30} Backward Pass Stats {'=' * 30}")
        data = []
        ...
        # 按照总耗时排序
        df = pd.DataFrame(data).sort_values(by="Total Time (ms)", ascending=False)
        # print(df.to_string(index=False))

        # 打印完后可以清空，准备下一个 Epoch
        self.stats.clear()
...
class NLLLossWithLabelSmoothing(nn.Module):
    """
    NLLLoss + label_smoothing, compatible with torch.nn.NLLLoss IO shapes.

    Input:
        log_probs: (N, C, d1, d2, ...)  -- must be log-probabilities (output of log_softmax)
        target:    (N, d1, d2, ...)     -- class indices
    """
    ...
    def forward(self, log_probs: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        if log_probs.dim() < 2:
            raise ValueError(f"log_probs must be (N,C,...) got {tuple(log_probs.shape)}")

        N, C = log_probs.shape[0], log_probs.shape[1]
        if target.shape[0] != N or target.shape[1:] != log_probs.shape[2:]:
            raise ValueError(
                f"target shape {tuple(target.shape)} must be (N, d1, d2, ...) == "
                f"{(N,) + tuple(log_probs.shape[2:])}"
            )

        # flatten spatial dims: (N, C, S) and (N, S)
        lp = log_probs.view(N, C, -1)
        t = target.view(N, -1).long()

        ignore = (t == self.ignore_index)  # (N, S)

        # gather needs valid indices; set ignored positions to 0 to avoid OOB
        t_safe = t.clone()
        t_safe[ignore] = 0

        # nll: -log p(y)
        nll = -lp.gather(dim=1, index=t_safe.unsqueeze(1)).squeeze(1)  # (N, S)

        # optional class weights (like nn.NLLLoss)
        if self.weight is not None:
            w_true = self.weight.gather(dim=0, index=t_safe.view(-1)).view_as(t_safe).float()  # (N, S)
            nll = nll * w_true

            # smooth loss uses weighted mean over classes: - sum_i w_i logp_i / sum_i w_i
            w = self.weight.view(1, C, 1)  # (1,C,1)
            smooth = -(lp * w).sum(dim=1) / (w.sum(dim=1).clamp_min(1e-12))  # (N, S)
        else:
            # smooth loss: - mean_i logp_i
            smooth = -lp.mean(dim=1)  # (N, S)
        ...
        # ignore_index positions contribute 0 and are excluded from mean denom
        ...
        # mean: divide by number of non-ignored elements (matches NLLLoss semantics)
        denom = (~ignore).sum().clamp_min(1)
        return loss.sum() / denom

class FFSDP_Span_Road_Executor(object):
    '''
    ...
    def __init__(self, config, model, roadgat_data):
        ...
        # self.backwardTimer = BackwardTimer(self.model)
        ...
        self.criterion_mask = NLLLossWithLabelSmoothing(ignore_index=0, reduction="none", label_smoothing=0.1)
        ...
        # -- dp
        self.dp_type = self.config.get('dp_type', 'road') # road / gps
        self.dp_len = self.config.get('dp_len', None)
        if self.dp_type != 'road':
            raise ValueError('dp_type should be gps')
        # nextloc prediction
        self.criterion = torch.nn.CrossEntropyLoss(reduction='none', label_smoothing=0.1)
    ...
    def register_time_hooks(model):
        ...
        '''
        # 存储 hooks 的引用，方便后续移除（可选）
        hooks = []

        def pre_hook_fn(module, grad_output):
            # 使用 torch.cuda.synchronize() 确保获取的是真实的 GPU 时间
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            module.start_backprop_time = time.perf_counter()

        def hook_fn(module, grad_input, grad_output):
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            elapsed = time.perf_counter() - module.start_backprop_time

            # 获取模块名称（如果模块没有记录名称，则显示类名）
            name = getattr(module, '_full_name', module.__class__.__name__)
            print(f"Backward | {name:<20} | Time: {elapsed:.6f}s")

        # 遍历所有子模块
        for name, module in model.named_modules():
            # 记录全名到模块属性中，方便 hook 调用
            module._full_name = name

            # 只为没有子模块的底层算子注册（避免重复计算父模块的时间）
            # 如果你想看整个 Sequential 的时间，可以去掉这个 if 判断
            if len(list(module.children())) == 0:
                h1 = module.register_full_backward_pre_hook(pre_hook_fn)
                h2 = module.register_full_backward_hook(hook_fn)
                hooks.extend([h1, h2])
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        for epoch_idx in range(start_epoch, self.max_epochs):
            ...
            train_avg_loss, train_avg_acc, train_last_avg_acc = self._train_epoch(train_dataloader, epoch_idx)
            t1 = time.time()
            train_time.append(t1 - start_time)
            train_loss.append(train_avg_loss)
            train_acc.append(train_avg_acc)
            train_last_acc.append(train_last_avg_acc)
            self._logger.info("epoch complete!")

            self._logger.info("evaluating now!")
            t2 = time.time()
            eval_avg_loss, eval_avg_acc, eval_last_avg_acc = self._valid_epoch(eval_dataloader, epoch_idx, mode='Eval')
            end_time = time.time()
            eval_time.append(end_time - t2)
            eval_loss.append(eval_avg_loss)
            eval_acc.append(eval_avg_acc)
            eval_last_acc.append(eval_last_avg_acc)
            ...
            if (epoch_idx % self.log_every) == 0:
                message = 'Epoch [{}/{}] ({})  train_loss: {:.4f}, train_acc: {:.4f}, val_loss: {:.4f}, eval_acc: {:.4f}, lr: {:.6f}, {:.2f}s'. \
                    format(epoch_idx, self.max_epochs, (epoch_idx + 1) * num_batches,
                           train_avg_loss, train_avg_acc,
                           eval_avg_loss, eval_avg_acc,
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            # road_padding_mask (B, len_max_road), True保留，False遮盖
            road_X, road_Target, road_padding_mask, road_target_masks, road_traj_mat, road_dp_target, \
                gps_X, gps_padding_mask = batch # road_dp_target (B,dp_length)
            road_fuse_gps_ada = self.model.forward_on_span_road(
                    gps_X, gps_padding_mask,
                    road_X, road_padding_mask, road_traj_mat,
                    self.graph_dict
            ) # (B,L_r,Vocab_Size)

            road_valid_lengths = road_padding_mask.sum(dim=-1) # (B,L) -> (B)
            last_token_indices = torch.clamp(road_valid_lengths-1, min=0)
            last_road_embedding = road_fuse_gps_ada[torch.arange(road_valid_lengths.shape[0], device=road_fuse_gps_ada.device), last_token_indices] # (B, Vocab_Size)
            road_dp_target = road_dp_target[:,-1].reshape(-1) # (B)

            targets_l, target_masks_l = road_Target[..., 0], road_target_masks[..., 0]  # (B, T), (B, T)
            mean_loss_l, batch_loss_l, num_active_l = self._cal_loss(road_fuse_gps_ada, targets_l, target_masks_l)
            mean_loss_last, batch_loss_last, num_active_last = self._cal_loss_single(last_road_embedding, road_dp_target)
            mean_loss = mean_loss_last

            # road_dp_pred : (B, VocabSize)
            # road_dp_target: (B, 1)
            # batch_loss_list = self.criterion(road_dp_pred, road_dp_target)
            ...
            # gps_max/min.unsqueeze(0) -> (1,2)
            # gps_dp_pred = gps_dp_pred * (self.gps_max - self.gps_min).unsqueeze(0) + self.gps_min.unsqueeze(0)
            # gps_dp_target = gps_dp_target * (self.gps_max - self.gps_min).unsqueeze(0) + self.gps_min.unsqueeze(0)
            #
            # batch_loss_list = self.criterion(gps_dp_pred, gps_dp_target)
            ...
            with torch.no_grad():
                # total pred_len acc
                total_correct_l += self._cal_acc(road_fuse_gps_ada, targets_l, target_masks_l)
                total_active_elements_l += num_active_l.item()
                epoch_loss.append(mean_loss.item())  # add total loss of batch
                # last pred_len acc
                total_correct_last += self._cal_acc_single(last_road_embedding, road_dp_target)
                total_active_elements_last += num_active_last.item()
            ...
            post_fix = {
                ...
                'loss': mean_loss.item(),
                # "meter distance": mean_meter.item(),
                "Loc acc(%)": total_correct_l / total_active_elements_l * 100,
                "Last Loc acc(%)": total_correct_last / total_active_elements_last * 100,
            ...
            if i!=0 and i % self.log_batch == 0:
                ...
                # self._logger.info(self.backwardTimer.print_stats())

        epoch_loss = np.mean(epoch_loss)  # average loss per element for whole epoch
        # epoch_meter = np.mean(epoch_meter)
        total_correct_l = total_correct_l / total_active_elements_l * 100.0
        total_correct_last = total_correct_last / total_active_elements_last * 100.0
        print()
        self._logger.info("📈 Train: expid = {}, Epoch = {}, avg_loss = {}, total acc ={}%, last total acc={}%."
                          .format(self.exp_id, epoch_idx, epoch_loss, total_correct_l, total_correct_last))

        self._writer.add_scalar('Train loss', epoch_loss, epoch_idx)
        self._writer.add_scalar('Train acc', total_correct_l, epoch_idx)
        self._writer.add_scalar('Train Last acc', total_correct_last, epoch_idx)
        return epoch_loss, total_correct_l, total_correct_last  # , total_correct_l
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        epoch_loss = []  # total loss of epoch
        # epoch_meter = []
        total_correct_last = 0  # total top@1 acc for masked elements in epoch
        total_active_elements_last = 0  # total masked elements in epoch
        total_correct_l = 0  # total top@1 acc for masked elements in epoch
        total_active_elements_l = 0  # total masked elements in epoch
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="⏳ {} epoch={}".format(
                ...
                road_X, road_Target, road_padding_mask, road_target_masks, road_traj_mat, road_dp_target, \
                    gps_X, gps_padding_mask = batch  # road_dp_target (B,dp_len)
                road_fuse_gps_ada = self.model.forward_on_span_road(
                    gps_X, gps_padding_mask,
                    road_X, road_padding_mask, road_traj_mat,
                    self.graph_dict
                ...
                # mean_loss = batch_loss / num_active
                #
                # # update epoch meter and epoch loss
                # meters = haversine_distance(gps_dp_pred, gps_dp_target)  # (B,)
                # mean_meter = torch.mean(meters) # 1
                # epoch_meter.append(mean_meter.item())
                # epoch_loss.append(mean_loss.item())  # add total loss of batch

                if mode == 'Test':
                    # preds.append(predictions.cpu().numpy())
                    # labels.append(road_tte_targets.cpu().numpy())
                    # evaluate_input = {
                    #     'y_true': road_tte_targets,
                    #     'y_pred': predictions
                    # }
                    evaluate_input = {
                        'loc_true': road_dp_target.cpu().numpy(),  # (B,)
                        'loc_pred': last_road_embedding.cpu().numpy()  # (B,vocab_size)
                    }
            ...
            print()
            self._logger.info("📈 {}: expid = {}, Epoch = {}, avg_loss = {}, total_acc = {}%, last total acc={}%."
                              .format(mode, self.exp_id, epoch_idx, epoch_loss, total_correct_l, total_correct_last))
            self._writer.add_scalar('{} loss'.format(mode), epoch_loss, epoch_idx)
            # self._writer.add_scalar('{} meter'.format(mode), epoch_meter, epoch_idx)
            self._writer.add_scalar('{} acc'.format(mode), total_correct_l, epoch_idx)
            self._writer.add_scalar('{} last acc'.format(mode), total_correct_last, epoch_idx)

            if mode == 'Test':
                self.evaluator.save_result(self.evaluate_res_dir)
            return epoch_loss, total_correct_l, total_correct_last
    ...
    def _cal_loss(self, pred, targets, targets_mask):
        ...
        pred (B, T, V)
        targets (B, T)
        targets_mask(B, T) True遮盖，False保留；
    ...
    def _cal_loss_single(self, pred, targets):
        '''
        ...
        '''
        batch_loss_list = self.criterion_mask(pred, targets.long())
        batch_loss = torch.sum(batch_loss_list)
        num_active = torch.tensor(targets.shape[0], device=batch_loss.device)
        mean_loss = batch_loss / num_active
        return mean_loss, batch_loss, num_active
    ...
    def _cal_acc_single(self, pred, targets):
        '''
        ...
        '''
        lm_output = pred.argmax(dim=-1)  # (num_active,)
        correct_l = targets.eq(lm_output).sum().item()
        return correct_l
    ...
    def save_model(self, cache_name):
        ...
        ensure_dir(self.cache_dir)
        config = dict()
        if isinstance(self.model, MambaDp):
            config['model'] = self.model.cpu()
            config['optimizer_state_dict'] = self.optimizer.state_dict()
            torch.save(config, cache_name)
            self.model.gps_view.to(self.device)
    ...
    def save_model_with_epoch(self, epoch):
        ...
        config = dict()
        if isinstance(self.model, MambaDp):
            config['model'] = self.model.state_dict()
            config['optimizer_state_dict'] = self.optimizer.state_dict()
            config['epoch'] = epoch
