...
class FFSDP_Road_Executor(object):
    '''
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            road_X, road_padding_mask, road_traj_mat, road_dp_target,\
                gps_X, gps_padding_mask = batch # gps_targets: (B,2)
            road_dp_pred = self.model(
            ...
            with torch.no_grad():
                ...
                correct = road_dp_pred.argmax(dim=-1).eq(road_dp_target).sum().item()
                total_correct += correct
                total_active_elements += num_active
                epoch_loss.append(batch_loss.item())  # add total loss of batch
            ...
            post_fix = {
                ...
                "Loc acc(%)": total_correct / total_active_elements * 100,
        ...
        print()
        self._logger.info("📈 Train: expid = {}, Epoch = {}, avg_loss = {}, total acc ={}%."
                          .format(self.exp_id, epoch_idx, epoch_loss, total_correct))
        self._writer.add_scalar('Train loss', epoch_loss, epoch_idx)
        self._writer.add_scalar('Train acc', total_correct, epoch_idx)
        return epoch_loss, total_correct  # , total_correct_l
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="⏳ {} epoch={}".format(
                ...
                if mode == 'Test':
                    ...
                    evaluate_input = {
                        ...
                        'loc_pred': road_dp_pred.cpu().numpy()  # (B,vocab_size)
                    }
                    self.evaluator.collect(evaluate_input)
                    # self._logger.info( self.evaluator.evaluate() )

                batch_loss_list = self.criterion(road_dp_pred, road_dp_target)  # (B)
                batch_loss = torch.sum(batch_loss_list)
                num_active = len(batch_loss_list)
                mean_loss = batch_loss / num_active
            ...
            # total_correct = total_correct / total_active_elements * 100.0
            print()
            self._logger.info("📈 {}: expid = {}, Epoch = {}, avg_loss = {}, total_acc = {}%"
                              .format(mode, self.exp_id, epoch_idx, epoch_loss, total_correct))
