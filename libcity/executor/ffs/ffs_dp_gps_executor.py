...
from libcity.model.ffs import MambaDp, MambaDpGps
...
class FFSDP_Gps_Executor(object):
    '''
    ...
    def __init__(self, config, model, roadgat_data):
        ...
        if self.initial_ckpt:
            self.load_model(self.initial_ckpt)
        ...
        self.lat_min = self.config.get('lat_min', 0)
        self.lat_max = self.config.get('lat_max', 0)
        self.lng_min = self.config.get('lng_min', 0)
        self.lng_max = self.config.get('lng_max', 0)
        self.gps_min = torch.tensor((self.lng_min, self.lat_min),dtype=torch.float32).to(self.device)
        self.gps_max = torch.tensor((self.lng_max, self.lat_max),dtype=torch.float32).to(self.device)
    ...
    def train(self, train_dataloader, eval_dataloader, test_dataloader=None):
        ...
        for epoch_idx in range(start_epoch, self.max_epochs):
            ...
            train_avg_loss, train_avg_meter = self._train_epoch(train_dataloader, epoch_idx)
            t1 = time.time()
            train_time.append(t1 - start_time)
            train_loss.append(train_avg_loss)
            train_meter.append(train_avg_meter)
            self._logger.info("epoch complete!")

            self._logger.info("evaluating now!")
            t2 = time.time()
            eval_avg_loss, eval_avg_meter = self._valid_epoch(eval_dataloader, epoch_idx, mode='Eval')
            end_time = time.time()
            eval_time.append(end_time - t2)
            eval_loss.append(eval_avg_loss)
            eval_meter.append(eval_avg_meter)
            ...
            if (epoch_idx % self.log_every) == 0:
                message = 'Epoch [{}/{}] ({})  train_loss: {:.4f}, train_meter: {:.4f}, val_loss: {:.4f}, eval_meter: {:.4f}, lr: {:.6f}, {:.2f}s'. \
                    format(epoch_idx, self.max_epochs, (epoch_idx + 1) * num_batches,
                           train_avg_loss, train_avg_meter,
                           eval_avg_loss, eval_avg_meter,
        ...
        self._draw_png([(train_loss, eval_loss, 'loss'), (train_meter, eval_meter, 'acc'), (lr_list, 'lr')])
        return min_val_loss
    ...
    def _train_epoch(self, train_dataloader, epoch_idx):
        ...
        for i, batch in tqdm(enumerate(train_dataloader), desc="⏳ Train epoch={}".format(
            ...
            road_X, road_padding_mask, road_traj_mat, \
                gps_X, gps_padding_mask, gps_dp_target = batch # gps_targets: (B,2)
            gps_dp_pred = self.model(
        ...
        print()
        self._logger.info("📈 Train: expid = {}, Epoch = {}, avg_loss = {}, meter distance ={}m."
                          .format(self.exp_id, epoch_idx, epoch_loss, epoch_meter))
        self._writer.add_scalar('Train loss', epoch_loss, epoch_idx)
        self._writer.add_scalar('Train meter distance', epoch_meter, epoch_idx)
        return epoch_loss, epoch_meter  # , total_correct_l
    ...
    def _valid_epoch(self, eval_dataloader, epoch_idx, mode='Eval'):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_dataloader), desc="⏳ {} epoch={}".format(
                ...
                if mode == 'Test':
                    preds.append(gps_dp_pred.cpu().numpy())
                    labels.append(gps_dp_target.cpu().numpy())
                    evaluate_input = {
                        'y_true': torch.zeros_like(meters), # 默认应该0误差
                        'y_pred': meters,
                    }
            ...
            print()
            self._logger.info("📈 {}: expid = {}, Epoch = {}, avg_loss = {},  meter distance ={}m."
                              .format(mode, self.exp_id, epoch_idx, epoch_loss, epoch_meter))
    ...
    def save_model(self, cache_name):
        ...
        ensure_dir(self.cache_dir)
        config = dict()
        if isinstance(self.model, MambaDpGps):
