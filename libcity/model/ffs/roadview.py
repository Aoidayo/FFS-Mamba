...
class RoadView(nn.Module):
    def __init__(self, config, road_gat_data):
        super(RoadView, self).__init__()
        self.config = config
        ...
        # from libcity.model.ffs.road_lg_embedding import RoadLG_Embedding
        from libcity.model.pm.bert_embedding_roadgm import BERTEmbedding
        # from libcity.model.pm.bert_embedding import BERTEmbedding
        # -- [pre model] embedding for BERT, sum of ... embeddings
        ...
        from libcity.model.pm.MixerModel import MixerModel
        self.mamba_blocks = TrajMixerModel(
            d_model = self.mamba_road_d_model,
            n_layer = self.mamba_road_n_layer,
            aux_feature_size = self.mamba_road_d_model,
            device = self.device,
            dtype = torch.float32,
        )


        # self.mamba_blocks_fwd = MixerModel(
        #     d_model= 256,        # embedding dim
        #     n_layer= self.mamba_road_n_layer,          # Mamba blocks (or Mamba+Attn)
        #     d_intermediate=512,
        #     vocab_size=0,       # 不需要 tokenizer 时设为 0
        #     input_dim=  self.mamba_road_d_model,  # e.g., gps feature dim (lat, lon, speed, time, ...)
        #     ssm_cfg={
        #         "layer": "Mamba1",       # 使用 Mamba1 (默认)
        #     },
        #     # attn_layer_idx=[2],          # 第 2 层插一个 Attention
        #     # attn_cfg={
        #     #     "num_heads": 8,
        #     # },
        #     rms_norm=True,
        ...
        # self.mamba_blocks_bwd = MixerModel(
        #     d_model=256,  # embedding dim
        #     n_layer=self.mamba_road_n_layer,  # Mamba blocks (or Mamba+Attn)
        #     d_intermediate=512,
        #     vocab_size=0,  # 不需要 tokenizer 时设为 0
        ...
        # self.mamba_blocks = TrajMixerModel2(
        #     d_model = self.mamba_road_d_model,
        #     n_layer= self.mamba_road_n_layer,
        #     d_intermediate=0,
        #     aux_feature_size = 0,
        #     d_state=self.mamba_road_d_state,
        #     headdim=self.mamba_road_head_dim,
        #     d_inner=self.mamba_road_d_inner,
        ...
        #         TransformerBlock(
        #             d_model=self.transformer_d_model,
        #             attn_heads=self.transformer_attn_heads,
        #             feed_forward_hidden=self.transformer_feed_forward_hidden,
        #             drop_path=enc_dpr[i],
        #             attn_drop=self.transformer_attn_drop,
        #             dropout=self.transformer_dropout,
        ...
        #         )
        #         for i in range(self.transformer_n_layers)
        #      ]
        # )

        # self.local_conv = nn.Conv1d(in_channels=self.transformer_d_model, out_channels=self.transformer_d_model, kernel_size=3, padding=1)
    ...
    def forward(self, x, padding_masks, batch_temporal_mat=None
        ...
        disc_feas = self.reverse_padded_sequence(embedding_output, valid_lengths)

        # padding_masks_rev = torch.flip(padding_masks, dims=[1])
        out_rev = self.mamba_blocks(input_rev_clean, disc_feas)
