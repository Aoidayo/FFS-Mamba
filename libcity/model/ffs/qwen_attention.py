...
class Qwen3RotaryEmbedding(nn.Module):
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len < self.original_max_seq_len and self.max_seq_len_cached > self.original_max_seq_len:  # reset
            self.register_buffer("inv_freq", self.original_inv_freq,
                                 persistent=False)
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: Optional[int] = None):
        ...
        self.o_proj = nn.Linear(self.num_heads * self.head_dim,
                                self.hidden_size, bias=config.qkv_bias)
        if self.use_qk_norm:
    ...
    def forward(
        ...
        if past_key_value is not None:
            cache_kwargs = {"sin": sin, "cos": cos,
                            "cache_position": cache_position}  # Specific to RoPE models
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def __init__(self, config: Qwen3Config):
        ...
        self.layers = nn.ModuleList(
            [Qwen3DecoderLayer(config, layer_idx) for layer_idx in
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            diagonal_attend_mask = torch.arange(target_length,
            ...
            if config.sliding_window is not None:
                ...
                if not isinstance(past_key_values,
                    ...
                    sliding_attend_mask = torch.arange(target_length,
                                                       device=device) <= (
                                                  cache_position.reshape(-1,
                                                                         1) - config.sliding_window
                                          )
                    diagonal_attend_mask.bitwise_or_(sliding_attend_mask)
            causal_mask *= diagonal_attend_mask
            causal_mask = causal_mask[None, None, :, :].expand(batch_size, 1,
                                                               -1, -1)
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                sequence_lengths = torch.eq(input_ids,
                                            self.config.pad_token_id).int().argmax(
                    -1) - 1
