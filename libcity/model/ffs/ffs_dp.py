...
class FFSDP(nn.Module):
    def __init__(self, config, road_vocab_size):
        super(FFSDP, self).__init__()
        self.config = config
        self.output_size = road_vocab_size

        self.d_model = self.config['mamba_gps_d_model']
        self._logger = getLogger()
        self._logger.info("🔧 构建FFS-Mamba的下游Adapter FFSDP")
    ...
    def forward(self, gps_X, gps_padding_mask):
        ...
        '''
        traj_emb = self.gps_view(gps_X, gps_padding_mask) # (B, D)
