...
class MambaFuseViewCollateFn:
    ...
    def __call__(self, raw_batch):
        ...
        if self.dp_type == "road":
            road_X, road_padding_mask, road_traj_mat, road_dp_target = self.__call_for_road__(road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch, dp_len_road_batch)
            gps_X, gps_padding_mask = self.__call_for_gps__(gps_traj_batch, dp_len_gps_batch)
            return (
                road_X, road_padding_mask, road_traj_mat, road_dp_target,
    ...
    def __call_for_road__(self, road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch, dp_len_road_batch):
        ...
        road_dp_target = torch.zeros(batch_size, dtype=torch.long) # (B)
        for i in range(batch_size):
            ...
            if self.dp_type == 'road':
                ...
                road_dp_target[i] = torch.tensor(road_traj_batch[i][-1,0], dtype=torch.long)
        ...
        # 构建输入
        road_X[..., 0:1].masked_fill_(road_padding_mask.unsqueeze(-1) == 0, self.vocab.mask_index)  # mask
        road_X[..., 1:].masked_fill_(road_padding_mask.unsqueeze(-1) == 0, self.vocab.pad_index) # pad
        ...
        if self.dp_type == 'road':
            return (
                ...
                road_traj_mat.to(self.device), # (batch_size, len_max_road, len_max_road)
                road_dp_target.to(self.device), # (batch_size)
            )
        elif self.dp_type == 'gps':
            return (
                road_X.to(self.device),  # (batch_size, len_max_road, f_road) maskIndex遮盖vocab_road_id，padIndex遮盖特征
