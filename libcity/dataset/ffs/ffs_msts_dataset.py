...
class FFS_Msts_Dataset:
    ...
    def get_data(self):
        ...
        qry_dataset, tgt_dataset, all_dataset = self.__gen_dataset()
        self._logger.info('📈 Size of dataset[Qry Tgt All]: ' +
                          str(len(qry_dataset)) + '/' + str(len(tgt_dataset)) + '/' + str(len(all_dataset)))
        self._logger.info("🔄 生成 Dataloader!")
        return self.__gen_dataloader(qry_dataset, tgt_dataset, all_dataset)

    def __gen_dataset(self):
        qry_dataset = QryDataset(
            config=self.config,
            type='test',
            vocab=self.vocab,
        )
        tgt_dataset = TgtDataset(
            config=self.config,
            type='test',
            vocab=self.vocab,
        )
        all_dataset = AllDataset(
            config=self.config,
            type='test',
            vocab=self.vocab,
        )
        return qry_dataset, tgt_dataset, all_dataset

    def __gen_dataloader(self, qry_dataset, tgt_dataset, all_dataset):
        '''
        ->  collate_fn
        Args:
            train_dataset:
        ...
        qry_dataloader = DataLoader(qry_dataset, batch_size=self.batch_size,
                                      num_workers=self.num_workers,
                                      shuffle=False,
                                      collate_fn=lambda raw_batch: self.collate_fn(raw_batch))
        tgt_dataloader = DataLoader(tgt_dataset, batch_size=self.batch_size,
                                     num_workers=self.num_workers, shuffle=False,
                                     collate_fn=lambda raw_batch: self.collate_fn(raw_batch))
        all_dataloader = DataLoader(all_dataset, batch_size=self.batch_size,
                                     num_workers=self.num_workers,
                                     shuffle=False,
                                     collate_fn=lambda raw_batch: self.collate_fn(raw_batch))
        return qry_dataloader, tgt_dataloader, all_dataloader

class QryDataset(Dataset):
    def __init__(self, config, type, vocab):
        ...
        # --
        self.cache_qry_gps_traj_list_path = self.config.get("cache_qry_gps_traj_list_path", None)
        self.cache_tgt_gps_traj_list_path = self.config.get("cache_tgt_gps_traj_list_path", None)
        self.cache_all_gps_traj_list_path = self.config.get("cache_all_gps_traj_list_path", None)
        self.cache_neg_indices_path = self.config.get('cache_neg_indices_path', None)
        self.cache_qry_tgt_indices_path = self.config.get('cache_qry_tgt_indices_path', None)
        ...
        self.qry_gps_traj_list = None
        self.tgt_gps_traj_list = None
        self.all_gps_traj_list = None
    ...
    def _load_data(self):
        ...
        if self.use_cache:
            ...
            prepare for MSTS
            """
            self.qry_gps_traj_list = np.load(self.cache_qry_gps_traj_list_path, allow_pickle=True)
            self._logger.info(f"🤗 Use Cache Query here: cache_qry_gps_traj_list_path loaded from {self.cache_qry_gps_traj_list_path}")
            self.qry_tgt_indices = np.load(self.cache_qry_tgt_indices_path, allow_pickle=True)
            self._logger.info(f"🤗 Use Cache Query Ids here: cache_qry_tgt_indices_path loaded from {self.cache_qry_tgt_indices_path}")
        else:
            raise RuntimeError("😲 必须 使用cache")
        # self.most_sim_index = np.load(self.most_sim_index_path, allow_pickle=True) # List

    def __getitem__(self, index):

        qry_gps_traj = self.qry_gps_traj_list[index]
        road_traj_index = self.qry_tgt_indices[index]
        road_traj = self.road_traj_list[road_traj_index]
        road_traj_mat = self.road_traj_mat_list[road_traj_index]
        ...
        return qry_gps_traj, road_traj, road_traj_mat, road_span_mlm_mask

    def __len__(self):
        return len(self.qry_gps_traj_list)
...
class TgtDataset(Dataset):
    ...
    def _load_data(self):
        ...
        if self.use_cache:
            ...
            prepare for MSTS
            """
            self.tgt_gps_traj_list = np.load(self.cache_tgt_gps_traj_list_path, allow_pickle=True)
            self._logger.info(f"🤗 Use Cache Tgt here: cache_tgt_gps_traj_list_path loaded from {self.cache_tgt_gps_traj_list_path}")
            self.qry_tgt_indices = np.load(self.cache_qry_tgt_indices_path, allow_pickle=True)
            self._logger.info(f"🤗 Use Cache Query Ids here: cache_qry_tgt_indices_path loaded from {self.cache_qry_tgt_indices_path}")
        else:
            raise RuntimeError("😲 必须 使用cache")
    ...
    def __getitem__(self, index):
        ...
        tgt_gps_traj = self.tgt_gps_traj_list[index]
        road_traj_index = self.qry_tgt_indices[index]
        road_traj = self.road_traj_list[road_traj_index]
        road_traj_mat = self.road_traj_mat_list[road_traj_index]
        road_span_mlm_mask = noise_mask(road_traj, self.masking_ratio, self.avg_mask_len, self.masking_mode, exclude_feats=None, add_cls=False)
        ...
        # return gps_traj, road_traj, road_traj_mat, road_span_mlm_mask #, \
        # aug_gps_traj # , aug_road_traj, aug_road_traj_mat, aug_road_span_mlm_mask
        return tgt_gps_traj, road_traj, road_traj_mat, road_span_mlm_mask

    def __len__(self):
        return len(self.tgt_gps_traj_list)
...
class AllDataset(Dataset):
    ...
    def _load_data(self):
        ...
        if self.use_cache:
            ...
            prepare for MSTS
            """
            self.all_gps_traj_list = np.load(self.cache_all_gps_traj_list_path, allow_pickle=True) # list len=20000, each (traj_i_len, feature_dim)
            self._logger.info(f"🤗 Use Cache All here: cache_all_gps_traj_list_path loaded from {self.cache_all_gps_traj_list_path}")
        else:
            raise RuntimeError("😲 必须 使用cache")
        # self.most_sim_index = np.load(self.most_sim_index_path, allow_pickle=True) # List
    ...
    def __getitem__(self, index):
        ...
        all_gps_traj = self.all_gps_traj_list[index]
        road_traj = self.road_traj_list[index]
        road_traj_mat = self.road_traj_mat_list[index]
        road_span_mlm_mask = noise_mask(road_traj, self.masking_ratio, self.avg_mask_len, self.masking_mode, exclude_feats=None, add_cls=False)
        return all_gps_traj, road_traj, road_traj_mat, road_span_mlm_mask

    def __len__(self):
        return len(self.all_gps_traj_list)
...
class MambaFuseViewCollateFn:
    ...
    def __call__(self, raw_batch):
        ...
        '''
        gps_X, gps_padding_mask = self.__call_for_gps__(gps_traj_batch) # ,gps_tte_targets
