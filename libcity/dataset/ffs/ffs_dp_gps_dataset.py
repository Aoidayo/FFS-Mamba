...
class MambaFuseViewCollateFn:
    ...
    def __call__(self, raw_batch):
        ...
        if self.dp_type == "road":
            road_X, road_padding_mask, road_traj_mat, road_dp_target = self.__call_for_road__(road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch)
            ...
            road_X, road_padding_mask, road_traj_mat = self.__call_for_road__(road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch)
            gps_X, gps_padding_mask, gps_dp_target = self.__call_for_gps__(gps_traj_batch)
            return (
    ...
    def __call_for_road__(self, road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch):
        ...
        for i in range(batch_size):
            ...
            if self.dp_type == 'road':
                road_X[i, :road_end-self.dp_len, :] = torch.tensor(road_traj_batch[i][:road_end-self.dp_len, :], dtype=torch.float32)
                road_traj_mat[i, :road_end-self.dp_len, :road_end-self.dp_len] = \
                    torch.tensor(road_traj_mat_batch[i][:road_end-self.dp_len, :road_end-self.dp_len], dtype=torch.float32)
                road_dp_target[i] = torch.tensor(road_traj_batch[i][-1,0], dtype=torch.long)
                road_valid_lengths[i] = road_end - self.dp_len
            elif self.dp_type == 'gps':
                road_X[i, :road_end, :] = torch.tensor(road_traj_batch[i][:road_end, :], dtype=torch.float32)
                road_traj_mat[i, :road_end, :road_end] = \
                    torch.tensor(road_traj_mat_batch[i][:road_end, :road_end], dtype=torch.float32)
            else:
    ...
    def __call_for_gps__(self, gps_traj_batch):
        ...
        for i in range(batch_size):
            ...
            gps_X[i, :gps_end-self.dp_len, :] = torch.tensor(gps_traj_batch[i][:gps_end-self.dp_len, :], dtype=torch.float32)
            if self.dp_type == "gps":
                ...
                raise RuntimeError('dp_type must be "road" or "gps"')
            gps_valid_lengths[i] = gps_end - self.dp_len
