...
class FFSTTEInnerDataset(Dataset):
    def __init__(self, config, type, vocab):
        ...
        # -- 解决Road和GPS信息互相泄露的问题
        self.cache_gps_traj_road_list_path = self.config.get("cache_gps_traj_road_list_path",None)
        self.cache_gps_traj_road_list_path = self.cache_gps_traj_road_list_path[:-4] + f"_{type}.pkl"
        ...
        #
        self.dp_type = self.config.get('dp_type', None)
        self.dp_use = self.config.get('dp_use', None)
        self.dp_len = self.config.get('dp_len', None)
        self.dp_ratio = self.config.get('dp_ratio', None)

        # -- 加载
        self.gps_traj_list = None
        self.road_traj_list = None
        self.road_traj_mat_list = None
        self.gps_traj_road_list = None
        self._load_data()
    ...
    def _load_data(self):
        ...
        if self.use_cache:
            ...
            self.gps_traj_road_list = pickle.load(open(self.cache_gps_traj_road_list_path, 'rb'))
            self._logger.info(f"🤗 Use Cache 20w here: gps_traj_road_list loaded from {self.cache_gps_traj_road_list_path}")
        else:
    ...
    def __getitem__(self, index):
        ...
        # -- for dp_type: road
        # road_last = self.road_traj_list[index][0][-1] # last road
        # vocab_road_last = self.vocab.roadIndex2vocabIndexDict.get(road_last, self.vocab.unk_index)
        ...
        dp_len_road, dp_len_gps = self.compute_dp_len(index)
        road_span_mlm_mask = noise_mask_last_pred_len(road_traj, dp_len_road)
        return gps_traj, road_traj, road_traj_mat, road_span_mlm_mask, dp_len_road, dp_len_gps

    def __len__(self):
        return len(self.gps_traj_list)

    def compute_dp_len(self, index):
        dp_len_gps = None
        dp_len_road = None
        gps_traj_road = self.gps_traj_road_list[index]
        road_traj_road = self.road_traj_list[index]

        if self.dp_use == 'ratio':
            # dp_len_road = int(len(road_traj_road) * self.dp_ratio)
            # dp_len_gps = self.last_n_runs_info(gps_traj_road, dp_len_road)
            dp_len_gps = int(len(gps_traj_road) * self.dp_ratio)
            dp_len_road = self.count_alg(gps_traj_road[-dp_len_gps:])
            return dp_len_road, dp_len_gps
        else:
            if self.dp_type == 'gps':
                dp_len_gps = self.dp_len
                dp_len_road = self.count_alg(gps_traj_road[-dp_len_gps:])
                # dp_len_road = len(list(set(gps_traj_road[-dp_len_gps:])))
            elif self.dp_type == 'road':
                dp_len_road = self.dp_len
                # dp_len_gps = self.dp_len
                dp_len_gps = self.last_n_runs_info(gps_traj_road, dp_len_road)
                total_len_gps = len(gps_traj_road)
                if dp_len_gps == total_len_gps:
                    raise RuntimeError("dp_len_gps != total_len_gps")
        return dp_len_road, dp_len_gps

    def count_alg(self, a):
        '''
        统计连续不重复的个数
        Parameters
        ----------
    ...
    def remove_tail_unique_blocks(self, a, remove_size):
        """
        从列表尾部开始，返回包含 remove_size 个不同元素的连续后缀
        """
        if remove_size <= 0:
            return a
        ...
        for i in range(len(a) - 1, -1, -1):
            seen.add(a[i])
            if len(seen) >= remove_size:
                cut_idx = i
                break

        return len(a[cut_idx:])

    def last_n_runs_info(self, arr, n_runs=5):
        """
        从 arr 尾部开始，获取最后 n_runs 个连续相同元素块的信息

        Returns:
            runs: List[dict], 每个 dict 包含:
                - value
                - start_idx (inclusive)
                - end_idx   (inclusive)
                - length
            total_length: 所有 runs 的长度之和
        """
        runs = []
        i = len(arr) - 1

        while i >= 0 and len(runs) < n_runs:
            val = arr[i]
            end = i
        ...
        total_length = sum(r["length"] for r in runs)

        # runs 是从后往前找到的，反转一下更直观
        runs.reverse()
    ...
    def data_processing_for_gps(self, origin_df):
        ...
        for i in tqdm(range(math.floor(origin_df.shape[0])), desc='MambaFuseViewInnerDataset: 处理GPSView ...'):
            ...
            one_gps_angle_delta_list = (np.array(one_gps_angle_delta_list, dtype='float') - delta_min) / (
                        delta_max - delta_min)
            # road
            one_gps_road_list = one_traj['gps_road_list']
            traj_fea = np.array(
                [
...
class MambaFuseViewCollateFn:
    def __init__(self, config, vocab):
        ...
        self.dp_type = self.config.get('dp_type', None)
        self.dp_len = self.config.get('dp_len', None)
        if self.dp_type is None and self.dp_len is None:
            raise RuntimeError('dp_type and dp_len are None? ')
        if self.dp_type == "road":
            self._logger.info("dp type: road")
        elif self.dp_type == 'gps':
            self._logger.info("dp_type: gps")
            self.lng_max = self.config.get('lng_max', None)
            self.lat_max = self.config.get('lat_max', None)
            self.lng_min = self.config.get('lng_min', None)
            self.lat_min = self.config.get('lat_min', None)
    ...
    def __call__(self, raw_batch):
        ...
        dp_len_road_batch, dp_len_gps_batch = [], []
        for row in raw_batch:
            ...
            dp_len_road_batch.append(row[4])
            dp_len_gps_batch.append(row[5])


        # gps_traj_batch, road_last_batch = [], []
        # for row in raw_batch:
        #     gps_traj_batch.append(row[0])
        #     road_last_batch.append(row[1])
        ...
        road_dp_target.to(self.device), # (batch_size) 最后一个路段id
        -- Gps View
        gps_X.to(self.device), # (batch_size, len_max_gps, f_gps)
        gps_padding_mask.to(self.device) # (batch_size, len_max_gps)
        gps_dp_target.to(self.device) # (batch_size, 2)
        '''
        road_X, road_padding_mask, road_traj_mat, road_dp_target = None, None, None, None
        gps_X, gps_padding_mask, gps_dp_target = None, None, None
        if self.dp_type == "road":
            road_X, road_Target, road_padding_mask, road_target_mask, road_traj_mat, road_dp_target = self.__call_for_road__(road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch, dp_len_road_batch)
            gps_X, gps_padding_mask = self.__call_for_gps__(gps_traj_batch, dp_len_gps_batch)
            return (
                road_X, road_Target, road_padding_mask, road_target_mask, road_traj_mat, road_dp_target,
                gps_X, gps_padding_mask,
            )
        elif self.dp_type == "gps":
            road_X, road_padding_mask, road_traj_mat = self.__call_for_road__(road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch, dp_len_road_batch)
            gps_X, gps_padding_mask, gps_dp_target = self.__call_for_gps__(gps_traj_batch, dp_len_gps_batch)
            return (
                road_X, road_padding_mask, road_traj_mat,
                gps_X, gps_padding_mask, gps_dp_target
            )
        else:
            raise RuntimeError(f"DP type {self.dp_type} is not supported.")

        # road_targets = [  self.vocab.roadIndex2vocabIndexDict.get(road_last, self.vocab.unk_index)  for road_last in road_last_batch]
        # road_targets = torch.tensor(road_last_batch, dtype=torch.long, device=self.device)
        # return (
        #     # gps_X, # (batch_size, len_max_gps, f_gps)
        #     # gps_padding_mask, # (batch_size, len_max_gps), True遮盖，False保留
        #     # road_targets , # (batch_size)
        #     # gps_dp_targets, # (batch_size, 2) gps轨迹的预测路段/gps点
    ...
    def __call_for_road__(self, road_traj_batch, road_traj_mat_batch, road_span_mlm_mask_batch, dp_len_road_batch):
        ''' 处理raw_batch 为批量
        ...
        road_target_masks = torch.zeros_like(road_X, dtype=torch.bool) # target_mask已经用False遮盖了最后五个
        road_traj_mat = torch.zeros(batch_size, max_road_len, max_road_len, dtype=torch.float32)
        # -- road_target_masks 遮盖最后五个路段
        road_dp_target = torch.zeros(batch_size, 1, dtype=torch.long) # (B,1)
        for i in range(batch_size):
            ...
            if self.dp_type == 'road':
                ...
                road_traj_mat[i, :road_end-dp_len_road_batch[i], :road_end-dp_len_road_batch[i]] = \
                    torch.tensor(road_traj_mat_batch[i][:road_end-dp_len_road_batch[i], :road_end-dp_len_road_batch[i]], dtype=torch.float32)
                road_dp_target[i] = torch.tensor(road_traj_batch[i][-1,0], dtype=torch.long) # (1)
                # road_valid_lengths[i] = road_end - dp_len_road_batch[i]
            elif self.dp_type == 'gps':
                road_X[i, :road_end-dp_len_road_batch[i], :] = torch.tensor(road_traj_batch[i][:road_end-dp_len_road_batch[i], :], dtype=torch.float32)
                road_traj_mat[i, :road_end-dp_len_road_batch[i], :road_end-dp_len_road_batch[i]] = \
                    torch.tensor(road_traj_mat_batch[i][:road_end-dp_len_road_batch[i], :road_end-dp_len_road_batch[i]], dtype=torch.float32)
            else:
                raise RuntimeError('dp_type must be "road" or "gps"')
            # road_target_masks[i, :road_end, :] = torch.tensor(road_span_mlm_mask_batch[i][:road_end, :], dtype=torch.bool)

            if road_valid_lengths[i] == 0:
                raise RuntimeError('road_valid_lengths[i] != 0')

        # (batch_size, len_max_road) True保留，False遮盖
        ...
        road_X[..., 1:].masked_fill_(road_target_masks[...,1:] == 1, self.vocab.pad_index) # pad
        # road_X.masked_fill_(road_padding_mask.unsqueeze(-1) == 0, self.vocab.pad_index)

        if self.dp_type == 'road':
            return (
                road_X.to(self.device), # (batch_size, len_max_road, f_road) maskIndex遮盖vocab_road_id，padIndex遮盖特征
                road_Target.to(self.device),  # (batch_size,  len_max_road, f_road)
                road_padding_mask.to(self.device),  # (batch_size, len_max_road) True保留，False遮盖
                road_target_masks.to(self.device), # (batch_size, len_max_road, f_road)
                road_traj_mat.to(self.device), # (batch_size, len_max_road, len_max_road)
                road_dp_target.to(self.device), # (batch_size, 5)
            )
        elif self.dp_type == 'gps':
            return (
                road_X.to(self.device),  # (batch_size, len_max_road, f_road) maskIndex遮盖vocab_road_id，padIndex遮盖特征
                road_padding_mask.to(self.device),  # (batch_size, len_max_road) True遮盖，False保留
                road_traj_mat.to(self.device),  # (batch_size, len_max_road, len_max_road)
            )
        else:
            raise RuntimeError('dp_type must be "road" or "gps"')
    ...
    def __call_for_gps__(self, gps_traj_batch, dp_len_gps_batch):
        '''处理GpsView的raw_batch
        ...
        gps_X = torch.zeros(batch_size, max_gps_len, gps_traj_batch[0].shape[-1], dtype=torch.float32)# 这里-1过滤gps_road
        # -- gps_tte_targets
        # gps_tte_targets = []
        # -- gps dp targets
        gps_dp_target = torch.zeros(batch_size, 2, dtype=torch.float32)
        gps_v_stalls = torch.zeros(batch_size, max_gps_len)  # (B, T)
        for i in range(batch_size):
            gps_end = min(gps_valid_lengths[i], max_gps_len)
            # dp_len_gps_batch[i] = max(dp_len_gps_batch[i]-10, 10)
            # len_max_gps, f
            # -- 只使用前[:gps_end-dp_len]个
            gps_X[i, :gps_end-dp_len_gps_batch[i], :] = torch.tensor(gps_traj_batch[i][:gps_end-dp_len_gps_batch[i], :], dtype=torch.float32)
            ...
            if self.dp_type == "gps":
                # 遮盖gps信息
                gps_dp_target[i,:] =  torch.tensor(gps_traj_batch[i][-1, 5:7], dtype=torch.float32)
            elif self.dp_type == 'road':
                # pass不用遮盖
                pass
            else:
                raise RuntimeError('dp_type must be "road" or "gps"')
            gps_valid_lengths[i] = gps_end - dp_len_gps_batch[i]
            if gps_valid_lengths[i] == 0:
                raise RuntimeError('gps_valid_lengths[i] != 0:')

        # (batch_size, len_max_gps)
        # gps_valid_lengths[i] = gps_end - self.dp_len 这边不减也没关系，本身就已经是zeros了
        ...
        '''
        gps_X：(batch_size, len_max_gps, f_gps) gps_X的嵌入，值得注意valid_len的后dp_len被遮盖
        gps_padding_mask: (batch_size, len_max_gps) True保留，False遮盖；由valid_len-dp_len生成padding_mask
        gps_dp_target: (batch_sized, 2) dp的预测值，归一化过的gps值
        '''
