...
class MlmGpsRoadContraDataset:
    def __init__(self, config):
        '''MlmGpsRoadContraDataset
        - MLM 任务
        - GpsRoad Contra：GPS resample 1/3 -> Road
        '''
...
class MambaFuseViewInnerDataset(Dataset):
    def __init__(self, config, type, vocab):
        ...
        self.cache_gps_resample_list_path = config.get("cache_gps_resample_list_path")
        ...
        self.cache_gps_resample_list_path = self.cache_gps_resample_list_path[:-4] + f"_{type}.pkl"
    ...
    def _load_data(self):
        ...
        if self.use_cache:
            ...
            self.gps_resample_list = pickle.load(open(self.cache_gps_resample_list_path, 'rb'))
            self._logger.info(f"🤗 Use Cache 20w here: gps_resample_list loaded from {self.cache_gps_resample_list_path}")
        else:
    ...
    def __getitem__(self, index):
        ...
        resmaple_index = np.array(self.gps_resample_list[index], dtype=bool)
        aug_gps_traj = gps_traj[resmaple_index]
